...
def createNode(node, data, i=[0], att=0):
    newNode = {}
    while(i[0]<len(data)):
        if(not '/' in data[i[0]][-1]): #new Node
            ...
            if(len(data[i[0]]) == 3 and data[i[0]][-1][-1] != '/'):
                newNode[data[i[0]][0]] = data[i[0]][1]
            else:
                att = GetAtt(data, i)
                newNode[data[i[0]][0]] = att
        else:
            return newNode
        i[0]+=1
    return newNode
...
with open('data.json', 'w') as f: #write dict to json
